...
const mapStateToProps = state => ({
  Ingredients:
    state.recipeDeck.HighlightedRecipes[0] &
    state.recipeDeck.HighlightedRecipes[1]
      ? state.recipeDeck.HighlightedRecipes
      : undefined,
  open: state.recipeDeck.open,
  recipeData: state.recipeDeck.recipeCompareData,
  dishname: state.clusters.RecipeName,
  title: state.recipeDeck.DisplayedRecipes.filter(d =>
    state.recipeDeck.HighlightedRecipes.includes(d.origin_id)
  )
    .map(d => d.title)
    .join(" vs ")
});
...
class _PopupComparison extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.recipeData)
      nextProps.recipeData.then(d => {
  ...
  render() {
    ...
    if (!this.state.loading) {
      //for actions
      actionData = {
        recipe1: this.state.data[0].actions,
        recipe2: this.state.data[1].actions
      };
      ...
      let intersection = [...r1].filter(x => r2.has(x));
