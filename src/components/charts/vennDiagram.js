...
export class VennDiagram extends React.Component {
  ...
  createVenn() {
    ...
    const lineHeight = 30,
      padding = 20,
      spreadValue = 0.525, //servers to mak ellipse look nice
      y = 200;
    ...
    let ry1 =
      Math.max(
        data.Recipe1.ingredients.length,
        data.Intersection.ingredients.length + 1
      ) *
      lineHeight *
      spreadValue;
    let ry2 =
      Math.max(
        data.Recipe2.ingredients.length,
        data.Intersection.ingredients.length + 1
      ) *
      lineHeight *
      spreadValue;
    ...
    if (!this.g1) this.g1 = this.svg.append("g").append("ellipse");
