...
export class Clusters extends React.Component {
  ...
  createMap() {
    ...
    cluster.map(
      d =>
        (clusterByNo[d.cluster_no] = clusterByNo[d.cluster_no]
          ? [...clusterByNo[d.cluster_no], d]
          : [d])
    );
    ...
    var x = d3
      .scaleLinear()
      .domain([
        d3.min(cluster.map(dot => dot.x)),
        d3.max(cluster.map(dot => dot.x))
      ])
      .range([0, width]);
    ...
    var y = d3
      .scaleLinear()
      .domain([
        d3.min(cluster.map(dot => dot.y)),
        d3.max(cluster.map(dot => dot.y))
      ])
      .range([0, height]);

    //for each cluster
    for (let [key, points] of Object.entries(clusterByNo)) {
      this.hull[key] = this.circles.append("path").attr("class", "hull" + key);

      const scaledPoints = points.map(dot => [x(dot.x), y(dot.y)]);
      const convexHull =
        scaledPoints.length < 3 ? scaledPoints : d3.polygonHull(scaledPoints);
      //Draw Hull
      this.hull[key]
        .datum(convexHull)
        .attr("d", function(d) {
        ...
        .attr(
          "visibility",
          this.props.activeCluster[key] ? "visible" : "hidden"
        )
        .attr("stroke-linejoin", "round")
        .on("mouseover", () => {
          this.hull[key].attr("fill", "gray").attr("stroke", "gray");
        })
        .on("mouseout", () => {
          this.hull[key]
            .attr("fill", colorArray[key])
  ...
  createCluster(points, node, r, div, x, y, key) {
    ...
    return this.circle[key]
      .attr(
        "d",
        d =>
          !this.props.centers.includes(d.recipe_id)
            ? d3
                .symbol()
      ...
      .attr(
        "fill",
        d =>
          this.props.highlights.includes(d.recipe_id)
            ? "black"
            : colorArray[key]
      )
      ...
      .attr("stroke", d => {
        let color = ";";
        if (this.props.centers.includes(d.recipe_id)) {
          color = "white";
        }
        if (this.props.selectedRecipes.includes(d.recipe_id)) {
          color = "black";
        }
        return color;
      })
      .attr(
        "stroke-width",
        d =>
          this.props.selectedRecipes.includes(d.recipe_id)
            ? 0.5 + "px"
            : 0.5 + "px"
      )
      ...
      .on("mouseover", d => {
        select(currentEvent.target).attr("d", d => {
          return !this.props.centers.includes(d.recipe_id)
            ? d3
                .symbol()
                .type(d3.symbolCircle)
                .size(64)()
