...
class Plot extends React.Component {
  ...
  createPlot() {
    ...
    let stack = d3
      .stack()
      .keys(this.props.selected_clusters)
      .value((d, key) => {
        return d[key];
      });
    this.props.histogram_detail.map((d, i) => {
      let temp = Object.assign(
        {},
        Array(this.props.selected_clusters.length).fill(0)
      );
      d.map(
        internal =>
          (temp[internal[0]] = temp[internal[0]] ? temp[internal[0]] + 1 : 1)
      );
      addedData.push(temp);
      return 0;
    });
    ...
    let y = d3
      ...
      .domain([
        0,
        d3.max(stack(addedData), function(d) {
    ...
    this.rect = stacks.selectAll("rect").data(d => d);
    ...
    this.rect
      ...
      .on("click", (d, i) => {
        ...
        this.props.addRecipes(
          det[i].filter(d => d[0] == clusterNo).map(d => d[1])
        );
      });
