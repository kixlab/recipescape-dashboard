...
const GraphTop = ({ name, topThree, action, clicked, addCombo }) => {
  return (
    <List horizontal>
      ...
      {topThree.map((item, i) => {
        let pair = {
            ingredient: action ? item : name,
            action: action ? name : item
          },
          highlight = false;
        if (clicked)
          highlight =
            clicked.ingredient == pair.ingredient &&
            clicked.action == pair.action;
        return (
          <List.Item
            style={{ cursor: "pointer" }}
            onClick={() => addCombo(action, pair)}
            key={item + i}
          >
            <Label size="mini" basic={highlight}>
              {item}
            </Label>
          </List.Item>
        );
...
const mapDispatchToProps = dispatch => ({
  addCombo: (action, ingredient_instruction) =>
    dispatch(setIngredientInstructionCombo(action, ingredient_instruction))
});
