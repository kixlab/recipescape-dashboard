...
const mapStateToProps = (state, ownProps) => {
  let overlay = ownProps.ingredient
    ? state.histograms.IngredientCombos.find(
        d => d.ingredient == ownProps.ingredient
      )
  ...
  return {
    ...
    overlayData: val,
    selected_clusters: state.clusters.ActiveClusters.map(
      (d, i) => (d ? i : -1)
    ).filter(d => d > -1)
  };
};
...
async function getVal(state, ownProps, overlay) {
  const val = await axios
    .post(BASE_URL + `histogram/${state.clusters.RecipeName}`, {
